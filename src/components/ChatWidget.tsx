    ...
    <div className="fixed bottom-5 right-5 z-[70] flex flex-col items-end gap-3 sm:bottom-6 sm:right-6">
      ...
      {!open && (
        <button
          ref={launcherRef}
          onClick={openWidget}
          className="chat-launcher-attention group flex min-h-18 items-center rounded-full bg-[#e72e8a] text-white shadow-[0_8px_28px_rgba(231,46,138,0.38)] transition hover:-translate-y-0.5 hover:bg-[#d81b7d] hover:shadow-[0_12px_34px_rgba(231,46,138,0.44)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-pink-200 active:translate-y-0 sm:min-h-14"
        ...
        >
          ...
          <span className="grid size-18 place-items-center sm:size-14">
...
function TeamAvatar({
  ...
  return (
    <div className={`relative ${dimensions} shrink-0`} aria-hidden="true">
      ...
      {online && (
        <span
          className={`absolute bottom-0.5 right-0.5 rounded-full bg-emerald-500 ring-2 ring-white ${
            size === "lg" ? "size-3.5 sm:size-2.5" : "size-2.5"
          }`}
        />
      )}
    </div>
